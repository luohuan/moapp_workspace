...
def main():
    with MoApp(appid='wxaa8fe119f5c62d23', name='好友回忆墙',withLogin=True):
        with Page(name='begin',enableShare=True, onReady=beginReady):
            with Box(name='entrance',pos=[0,0], hidden=True,width=750):
                ...
                starblink()
                ImageAvatar(name='imageAvatar',pos=['center', 10],  size=[120, 120], borderRadius='50%')
                Text(name='tip', text='【设置你的满分好友标准】',pos=['center', 130],color='#FFD886')
                ...
                Button(name='btn', text='确定',pos=['center',1030], type='plainDefault', openType='getUserInfo',border="1rpx solid #FFD886",boxShadow='-1px 15px 30px -12px black',fontSize='18px',backgroundColor="#FEB621",borderRadius='8px', onTap=indexsubmit)
        ...
        with Page(name='mainpage', background="#2A2C3B",onReady=mainPageReady, onPullDownRefresh=mainPageReady, barColor='#2a2c3b', barStyle='white', enableShare=True):
            ...
            with Box(pos=[0,0],position='relative'):
                ...
                Text(name='tip1',effect=ef55,hidden=True, text='目前还没有好友来测，快点击底部按钮，去邀请好友来回忆吧！', pos=['center', 580], color='#FFFFFF', fontSize=36)
                ...
                with List(name='guest_records', pos=[37, 961], marginBottom=100):
                    with Box(width=684, height='{item.height}rpx',border='1px solid #5F4270',borderRadius='2%',background='#121639',marginBottom=20):
                        ...
                        onTap=moui.confirm('温馨提示:','查看全部好友的选择(包括未来进入的好友),需支付金额9.9元',moui.request(checkDetailPay, host_id='{item.host_openid}', guest_id='{item.guest_openid}')))
...
async def beginReady(user, app, page, mo):
    ...
    page.hostName.text = '在你心中，要一起做过哪些事，才算是一百分的友情呢？'
...
async def indexsubmit(user, app, page, mo):
...
async def checkDetailPay(user, app, page, mo, params):
    ...
    mo.wxpay.pay('查看与ta的回忆', 9.9, onPaySuccess,  onPayFail)

async def onPaySuccess(user, app, page, mo):
    tmpRecord = mo.db.guests.find({'openid':page.data.guest_id,'hostopenid':page.data.host_id})
    mo.console('支付成功')
    mo.console(str(tmpRecord))
...
async def page1Ready(user, app, page, mo): 
    ...
    #     page.data.set('things_list', things_list)
    #     page.btn.hide()
        # mo.db.guests.administrator_clear_table()
        # return
...
async def submit(user, app, page, mo):
    ...
    if count in list(range(1,6)):
        ...
        words = random.choice([
                ...
                '一起跋山涉水，一起奔走风尘，风风火火的友情不用解释！',
                '时间越久，越能发现彼此的好，友谊之酒越醇香。'
                ])
        ...
        words = random.choice([
                ...
                '相知相守，你就是另一个我，我就是另一个你。',
                '天生搭档，势均力敌，要并肩一起走向人生巅峰呀！',
                '我们有着左右手的默契，打配合就能天下无敌！'
                ])
