...
DESCRIBE = [
    '两个人都充满了激情和火热，一旦相遇，友情的小火苗会立刻点燃，一拍即合。',
    '你们一个踏实可靠，一个激情似火，看似有些不同，实际却很互补，天生适合做彼此志同道合的朋友。',
    '你们大部分时候会比较合拍，但是有时也会难以应付对方，相处起来可能会觉得有点累。',
    '你们一个比较内向，一个激情多，耐性少。偶尔会有一些小摩擦，但患难时总能帮对方一把。',
    '你们性情相近，但又都很有想法，可能会产生磨擦，但有不同的声音，才能督促彼此共同进步。',
    '你们一个凡事追求完美；另一个则比较耿直冲动。但当自己真的犯错时，对方都会给你正确的批评和帮助。',
    '你们懂得彼此欣赏，呆在一起，可以互补长短，相得益彰。',
    '你们一个比较没有安全感。一个人则比较热情主动，所以你们的相遇，将会促成一段最美丽的友情。',
    '你们可能不常见面、聊天，但当你们迷茫时，对方的启发或许能给你柳暗花明的感觉。',
    '你们彼此互相欣赏，在逆境中拼搏时，对方能给你们带来了坚持到底的勇气，构成的友情也会非常地坚固。',
    '你们的相遇充满了缘分，伤心是可以彼此依靠，开心时能互相分享，遇见彼此，是最美丽的意外。',
    '你们的性格看起来可能并不一样，却充满了相互吸引的潜质，在生后中将会成为彼此最合拍的拍档。',
    '你们品性相投，无需刻意保持距离，计算尺度，只需素面朝天，就能通晓对方的心意。',
    '你们一个人偏向踏实稳重，另一个人则更会追求自由。你们虽互相吸引，但只有多沟通才能更了解彼此。',
    '你们俩有很多的共同点，都会追求稳重踏实和安定善良目标，相处起来十分融洽。',
    '你们都拥有强大的专注力、持久力、执著力，相遇的之后，一定会成为共学、共道的朋友。',
    '你们内心都拥有保守而踏实的本质，理性多过感性。彼此都绝对信任对方，属于可以共患难的朋友。',
    '你们性格略有不同，但恰好互相弥补。你们未必可倾诉心事，但生活上往往能互帮互助、相互照顾。',
    '你们两人最大的共同点就是拥有深情和忠诚的本性，可以经受共患难，同富贵的考验。 ',
    '你们相互影响，不断开拓新思维，你们的相处之道让彼此感到非常地舒适，相见恨晚。',
    '你们共同的优点就是踏实靠谱、务实稳重，不会天天寒暄，却会雪中送炭，不会日日问候，却会济困解危。',
    '你们性格上的差异，让你们常常意见不同，但是如果抱着学习的心态，却刚好可以取长补短，相得益彰。 ',
    '你们性格超合拍，如果可以保持沟通，相互理解，你们的感情会非常长久。',
    '你们对彼此的性格了若指掌，就如同左手和右手一样和谐。是彼此最信任的朋友。',
    '你们性格上稍有不同，彼此，你们将获得最独特的友情。'
]   
...
def makeShareImage():
    ...
    ret = mo.acode.getWxAcodeUrl(params)
...
def makeResultUI(name_header, x, y):
        with Box(pos=[x, y],size=[686, 1240]):
            ...
            Button(name='%shost_name' %name_header, pos=[70, 170],background='rgba(0,0,0,0)',size=[200,90],textAlign='center',color='#F56E6E', fontSize=24)
...
def makeResultUI2(name_header, x, y):
        with Box(pos=[x, y],size=[686, 1240]):
            Box(pos=[128, 74], size=[90, 90],border='0px solid #F56E6E',borderRadius='50%',background='#D03F3F')
            Image(name='%shost_avatar' %name_header,onTap=moui.request(changeName,state='host'),src='http://material.motimaster.com/appmaker/lijiong/2315.png', pos=[133, 79], size=[80, 80], borderRadius='50%',)
            Box(pos=[460, 74], size=[90, 90],border='0px solid #3E4DD2',borderRadius='50%',background='#3E4DD2')
            Image(name='%sguest_avatar' %name_header,onTap=moui.request(changeName,state='guest'),src='img/head2.jpg', pos=[464, 79], size=[80, 80], borderRadius='50%')
            Box(pos=[269, 142], size=[140, 2],border='2px solid #F56E6E',borderRadius='0%',background='#F17093')
            Text(name='%sscore' %name_header, text='87分', pos=['center', 58],color='#F56E6E', fontSize=42)
            Text(name='%shost_name_button' %name_header,hidden=True, border='5rpx solid #F56E6E',lineHeight=32, onTap=moui.request(changeName,state='host'),borderRadius=20,pos=[123, 170],size=[100,40],textAlign='center',color='#F56E6E', fontSize=24)
            Text(name='%sguest_name_button' %name_header, hidden=True,border='5rpx solid #3E4DD2',lineHeight=32,onTap=moui.request(changeName,state='guest'),borderRadius=20,pos=[454, 170],size=[100,40],textAlign='center',color='#3E4DD2', fontSize=24)
            Text(name='%shost_name' %name_header, hidden=True, pos=[123, 170],size=[100,40],textAlign='center',color='#F56E6E', fontSize=24)
            Text(name='%sguest_name' %name_header, hidden=True, pos=[454, 170],size=[100,40],textAlign='center',color='#3E4DD2', fontSize=24)
            Image(src='img/jisuan1.jpg', pos=[25, 253], size=[630, 96],effect=fadein(size=(0,1),d=1,t=1,c=1,p=1,s=0))
...
def changeName(user, app, page, mo, params):
    if mo.token.isHost() and params.state == 'host':
        page.change_name_mask.hidden = False
    elif not mo.token.isHost() and params.state == 'guest':
        page.change_name_mask.hidden = False
...
class match(Page):
    ...
    def UI(self):
        ...
        with Box(name='match_box', hidden=False,pos=[0, 155], width=750, background='#383838'):
            ...
            with Box(name='guest_list', hidden=True, pos=[33, 335], size=[686, 1140],border='1px solid #FFFFFF',borderRadius='0%',background='http://img.mogodeer.cn/images/diudiu/ceshi1/wnagge.jpg',boxShadow ='3px 3px 1px 3px rgba(208,63,63,0.9)',marginBottom=50):
                makeResultUI2('', 0, -20)                
                Text(name='wenan', background='white', text='', fontSize=27,pos=['center', 860], width=600)
            with Box(pos=[0, 1530]):
                with List(name='rest_guests', position='relative', border='0px solid #FFFFFF',borderRadius='0%'):        
                    with Box(pos=[33, 0], size=[686, 150],border='0px solid #FFFFFF',borderRadius='0%',background='#D96363',boxShadow ='2px 2px 1px 2px rgba(255,255,255,1)',marginBottom=50):                        
                        Image(src='{item.host_avatar}', pos=[41, 15], size=[80, 80], border='8rpx solid #FFFFFF',borderRadius='50%')                
                        Image(src='{item.guest_avatar}', pos=[205, 15], size=[80, 80], border='8rpx solid #FFFFFF',borderRadius='50%')
                        ...
                        Text(text='{item.host_name}',pos=[41, 110],size=[96,24],textAlign='center',color='#FFFFFF', fontSize=20)
                        Text(text='{item.guest_name}', pos=[202, 110],size=[96,24],textAlign='center',color='#FFFFFF', fontSize=20)
            ...
            Button(name='createbutton', text = '创建我的', position='fixed', left=195,bottom=30,size=[360,78],background = 'rgba(255,206,48,0.9)',
                lineHeight = 78,fontSize=30,fontWeight = 'bolder',borderRadius = '500px', color = '#3c3a59',boxShadow ='0px 0px 10px 5px rgba(0,0,0,0.5)',openType='getUserInfo', 
        ...
        #     AD(unitId='adunit-7f57f6c8c6c20904')
            ...
            with Mask(name='change_name_mask', locked=True, hidden=True):
                ...
                with Box(pos=[43, 300], size=[666, 500],border='1px solid #FFFFFF',borderRadius='0%',background='#383838',boxShadow ='3px 3px 1px 3px rgba(208,63,63,0.9)',marginBottom=50):
                    Text(name='text3',text='修改你的名字:', color='#FFFFFF', fontSize=40, pos=['center', 40])
                    Image(src='img/delete.png', onTap=self.deleteMask,size=[50,50],right=12, top=12)
                    Input(name='input_host_name', pos= ['center',150], size=[480,80],
                          placeholder = '请输入你的名字', border='2px solid #F24040',background='255 255 255 0.3',color='#F46060',textAlign='center')
                    Button(name='button1', text = '提交', pos=['center',338],size=[360,78],background = 'rgba(242,64,64,0.82)',
                                           lineHeight = 78,fontSize=36,fontWeight = 'bolder',borderRadius = '500px', 
                                           color = '#FFFFFF',boxShadow ='4px 8px 1px 3px rgba(0,0,0,0.2)',openType='getUserInfo',
                                           onTap = [moui.showLoading('加载中...'), commmitchangeName, moui.hideLoading()])
    ...
    def commmitchangeName():
        ...
        if mo.token.isHost():  #如果是主态
            mo.token.set('host_name', name)
            mo.token.set('host_headimg', user.avatarUrl)
            page.change_name_mask.hide()
            guests = mo.token.getGuestList()
            for guest in guests:
                ret = calcName(name, user.gender, guest['name'], guest['gender'])
                mo.token.setGuestData(guest['openid'],result=ret)

            mo.token.redirectTo('match',mode='match')
        else: # 如果是客人态
            host_name = mo.token.get('host_name')
            host = mo.token.getHostInfo()
            ret = calcName(host_name, host['gender'], user.name, user.gender)
    ...
    def onInit():
        ...
        if mo.token.isHost():
            ...
            page.host_name.hidden = True
            page.guest_name_button.hidden = True
            page.guest_name.hidden = False
        else:
            #if mo.token.isHost():
            page.host_name_button.hidden = True
            page.host_name.hidden = False
            page.guest_name_button.hidden = False
            page.guest_name.hidden = True
    ...
    def matchBoxInit():
        ...
        if len(guests)>0:
            ...
            page.host_name_button.text = mo.token.get('host_name')   
            page.guest_avatar.src = last_guest['avatarUrl']
            page.guest_name.text = last_guest['name']
            page.guest_name_button.text = last_guest['name']
            ...
            result_number = int(last_guest['result']['score'])
            wenan = '%s中的'%result_number
            num_set = set()
            ...
            for num in str(result_number):
                #if num !=0:
                if num in num_set:
                    wenan += NUMBERS['number'+num][result_number%(len(NUMBERS['number'+num]))] + '，'
                    num_set.discard(num)
    ...
    def showDetail(): #显示匹配细节
        ...
        # #num_set.remove('0')
        # for num in num_set:
        #     wenan2 += NUMBERS['number'+num][result_number%(len(NUMBERS['number'+num]))] + ','

        # wenan2 += '总得来说，'
        # wenan2 += DESCRIBE[int(guest['result']['score'])%len(DESCRIBE)]
        # page.wenan2.text = wenan2
        result_number = int(guest['result']['score'])
        wenan2 = '%s中的'%result_number
        num_set = set()
        for num in str(result_number):
            num_set.add(num)
        #num_set.remove('0')
...
class share(Page):
    ...
    def UI(self):
       ...
       Image(name='shareImage', pos=['center',23], size=[700,700])
