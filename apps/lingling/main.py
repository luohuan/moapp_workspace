def main():
        ...
        with Page(name='start',background='http://material.motimaster.com/appmaker/menlu/3645.jpg'):
            ...
            with Box(name='head',pos=['center',0]):
                Button(effect=pulse(t=1,c=0),text='开   始', size=[400, 90], pos=['center', 825], 
                    type='plainDefault',plain=True, 
                  openType='getUserInfo',onTap=gotoSecondPage,backgroundColor='#c64342',color='#FFFFFF',
                  fontSize='15px',lineHeight=90,borderRadius='15px',boxShadow='3px 3px 5px #888888',border='0px')
    ...
    ###################生成二维码##################
        with Page(name='matrixcode',navigationBarTitleText='分享', enableShare=True,
         ...
         onReady=[moui.showLoading('加载中'),CreatMatrixCode, moui.hideLoading()]):
             ...
             Button(effect=pulse(t=1,c=0),text='保存到相册', plain=True, pos=['center', 810], size=[456, 88],lineHeight=88,
               fontSize='15px',onTap=save,border='0px',borderRadius='20px',color='#FFFFFF',fontWeight='bold',
               background='http://material.motimaster.com/appmaker/menlu/2907.png')
             ShareButton(effect=pulse(t=1,c=0),text='邀请朋友玩', plain=True, pos=['center', 930], size=[456, 88],lineHeight=88,
        ...
        with Page(name='relationshipresult',background='http://material.motimaster.com/appmaker/menlu/2671.jpg'):
             ...
             with Box(size=[750,600]):
                 ...
                 Image(id='scoreimage',size=[500,500],pos=['center',10],
                    src='http://material.motimaster.com/appmaker/menlu/3612.png')
                 Image(id='scoreimage2',size=[500,500],pos=['center',10],effect=pulse(t=1,c=0),
                    src='http://material.motimaster.com/appmaker/menlu/3614.png')
                 ...
                 with ScrollBox(name='friendlist',scrollY=True,size=[623,550],pos=[76,690]):
                    with List(name='friends'):
                        with Box(size=[609,280],background='http://material.motimaster.com/appmaker/menlu/3438.png',borderRadius='50rpx',marginBottom= 8):
        ...
        with Page(name='friendsanswerlist',enableShare=True,
            ...
            with Box(size=[613,654],pos=['center',330],backgroundColor='#f0d668'):
                ...
                with ScrollBox(name='friendlist',scrollY=True,size=[623,500],pos=['center',150]):
                    Text(id='Alert',text='目前还没有好友和你匹配呦~快邀请好友匹配吧！',size=[500,280],fontSize='13px',
                ...
                with Box(size=[600,90],pos=['center',674]):
                    with Button(name='btn',effect=shake(t=2.5,c=0),text='邀请好友匹配', hidden=True, plain=True, pos=['center', 0], size=[255, 88],lineHeight=88,
                       fontSize='15px',border='0px',borderRadius='5px',color='#FFFFFF',fontWeight='bold',backgroundColor='#c64342',
                       boxShadow='3px 3px 5px #888888'):
                       this.onTap=findNewData
                    ShareButton(name='shareBtn',effect=shake(t=2.5,c=0),text='邀请好友匹配', hidden=True, plain=True, pos=['center', 0], size=[255, 88],lineHeight=88,
                       fontSize='15px',border='0px',borderRadius='5px',color='#FFFFFF',fontWeight='bold',backgroundColor='#c64342',
                       boxShadow='3px 3px 5px #888888')
...
async def entranceReady(user, app, page, mo):
    ...
    for item in normal_test_question:
      ...
      if len(item['answer']) != 4:
        mo.console(item)
    result=mo.db.hostAnswerData1.find({
      'hostid': user.id,
      })
    if len(result) >= 1:
      mo.console('找到了')
...
async def Delete(user, app, page, mo):
    items = mo.db.hostAnswerData1.find({ 
        'hostid': user.id,
        })
    ...
    if  len(items) != 0:
      for item in items:
        mo.console(item['id'])
        mo.db.hostAnswerData1.delete(item['id'])
...
async def GuestInfoLists(user,app,page,mo):
    # mo.setNavibarColor('#000000', '#f0d668')
    question=mo.db.hostAnswerData1.find({
      'hostid': user.id
      })
    ...
    page.share.options = {
        ...
        'host_id':host_id,
        'host_name':host_name,
        'host_avatarUrl':host_avatarUrl
    }
     #######改#####此处应该针对出题的题目id去获取用户的答案列表啊 
    page.friends.data=mo.db.guestAnswerData1.find({
      'host_id':user.id
      })
...
async def afterAnswer(user, app, page, mo):
    ...
    if index == 5:
        ...
        question_id = mo.db.hostAnswerData1.insert({
...
async def guestanswerQuestionReady(user, app, page, mo):
    ...
    if len(result) == 1:
      ...
      if questionInfo['hostid'] == user.id:
        ...
        mo.console('客人态')
        theguest=mo.db.guestAnswerData1.find({
...
async def guestafterAnswer(user, app, page, mo):
    ...
    if index == 5:
        ...
        mo.console(page.data.selected)
        guestAnswer_id=mo.db.guestAnswerData1.insert({
...
async def onCompareAnswer(user,app,page,mo):
    ...
    guestInfo = mo.db.guestAnswerData1.find({
    ...
    hostInfo = mo.db.hostAnswerData1.find({
      'hostid': page.options.hostuser_id
      })
    ...
    mo.db.guestAnswerData1.update(guestInfoID,{
        'score':score,
...
async def findHostData(user,app,page,mo):
    ...
    #   })
    # questionInfo = result[0]
    page.hostAnswerlist.data =  mo.db.hostAnswerData1.find({
      'id': page.options.question_id
      })  #    
async def deleteHostItemData(user, app, page, mo, params):
    # 删除
    mo.db.hostAnswerData1.delete(params.id)
    page.hostAnswerlist.data = mo.db.hostAnswerData1.find()
...
resulet_solgn=['确认过脑仁儿，你就是对的人','这么复杂的脑回路上都能遇到你','再靠近一点点我就跟你走','这位仁兄脑回路很清奇啊！']
share_solgn=['我们脑回路一样嘛？','脑回路相似度，我们俩能得几分？','听说我们脑回路很相似，测测能得几分吧！','脑回路匹配大作战，……']
